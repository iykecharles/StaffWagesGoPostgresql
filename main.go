...
func main() {
	...
	http.HandleFunc("/datavault", Auth(datavault))
	...
	http.HandleFunc("/insertdata", Auth(insertdata))
	http.HandleFunc("/insertdone", Auth(insertdone))
	...
	err = http.ListenAndServe("localhost:8000", context.ClearHandler(http.DefaultServeMux))
	if err != nil {
		log.Fatalln(err)
	}
}
...
func alterstafftable(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintln(w, "STAFF TABLE SUCCESSFULLY ALTERED", n)
